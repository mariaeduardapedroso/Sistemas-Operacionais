...
void imprima(char *frasePassada, ...)
{
    ...
    while (frasePassada[i] != '\0')
    {
        ...
        if (frasePassada[i] == '%')
        {
            ...
            case '.': // 1 2 3 4 5
                if ((frasePassada[i + 1] <= 53 && frasePassada[i + 1] >= 49) && frasePassada[i + 2] == 'f')
                {
                    double flutuante = va_arg(args, double);
                    buffer[0] = '\0';
            ...
            case '0': //.1 .2 .3 .4 .5
                if (frasePassada[i + 1] == '.' && (frasePassada[i + 2] <= 53 && frasePassada[i + 2] >= 49) && frasePassada[i + 3] == 'f')
                {
                    double flutuante = va_arg(args, double);
                    buffer[0] = '\0';
...
int main()
{
    ...
    // EXERCICIO 4
    imprima("Flutuante com pontos %.3f\n", flutuante1);
